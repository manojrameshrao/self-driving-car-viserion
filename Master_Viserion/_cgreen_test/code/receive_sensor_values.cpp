...
int checkSensorValues()
{
    uint8_t speed = 0;
    uint8_t direction = 0;
    int SENSOR_left_in = sensorValueLeft();
    int SENSOR_middle_in = sensorValueMiddle();
    int SENSOR_right_in = sensorValueRight();
    int retval = 0;
    if(SENSOR_middle_in <= MIN_MIDDLE_DIST) //if middle : 0 to 10
    {
        /*LD.setNumber(5);
        speed = 1; //break
    ...
    else if(SENSOR_middle_in > MIN_MIDDLE_DIST && SENSOR_middle_in <= MIDDLE_DIST) //middle detected 11 - 25
    {
        if(SENSOR_left_in <= CORNER_DIST && SENSOR_right_in <= CORNER_DIST )
        {
        ...
        else if(SENSOR_right_in <= CORNER_DIST) // right(0 - 25) + middle(11 - 25)
        {
           /* LD.setNumber(70);
            speed = 2; //slow
        ...
        else if(SENSOR_left_in <= CORNER_DIST)  //left(0 - 25) + middle (11 -25)
        {
           /* LD.setNumber(60);
            speed = 2; //slow
    ...
    {
       /*if(SENSOR_left_in <= MIN_CORNER_DIST && SENSOR_right_in <= MIN_CORNER_DIST)
        {
            //LD.setNumber()
            speed = 1; //brake
            direction = 0; // straight
        }*/
        if(SENSOR_right_in <= CORNER_DIST && SENSOR_left_in <= CORNER_DIST)
        {
        ...
        else if(SENSOR_right_in <= CORNER_DIST)
        {
        ...
        else if(SENSOR_left_in <= CORNER_DIST)
        {
